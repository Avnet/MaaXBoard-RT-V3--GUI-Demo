...
void setup_scr_screen0_MENU(lv_ui *ui){
	...
	lv_label_set_text(ui->screen0_MENU_label_1, " *** FACTORY TEST ONLY *** ******* DO NOT SHIP *******");
#else
...
#if defined(__FAT_BUILD__)
	lv_style_set_bg_color(&style_screen0_MENU_label_1_main, LV_STATE_DEFAULT, lv_color_make(0xff, 0, 0));
	lv_style_set_bg_grad_dir(&style_screen0_MENU_label_1_main, LV_STATE_DEFAULT, LV_GRAD_DIR_NONE);
#else
...
#if defined(__FAT_BUILD__)
	lv_obj_set_pos(ui->screen0_MENU_label_1, 130, 100);
#else
