...
{
	if (key <= 56) {
		struct hid_device keyboard_dev;
		keyboard_dev.dev_type = KEYBOARD_DEVICE;
		keyboard_dev.dev_btn = g_HostKeyboardTable[key][shift];
		xQueueSend(*hid_devices_queue, (void *) &keyboard_dev, 10);
	}
}
...
{
	custom_usb_host_keyboard_instance_t *keyboard_struct = (custom_usb_host_keyboard_instance_t *)param;
    usb_host_keyboard_instance_t *keyboardInstance = keyboard_struct->host_hid_keyboard;
    hid_devices_queue = keyboard_struct->hid_queue;
    g_HostHidKeyboard = keyboard_struct->host_hid_keyboard;
            ...
            if (g_HostHidKeyboard->configHandle == configurationHandle)
            {
                if ((g_HostHidKeyboard->deviceHandle != NULL) && (g_HostHidKeyboard->interfaceHandle != NULL))
                {
                    ...
                    if (g_HostHidKeyboard->deviceState == kStatus_DEV_Idle)
                    {
                        ...
                        usb_devices[1].deviceExist = 1;
                        USB_HostHelperGetPeripheralInformation(deviceHandle, kUSB_HostGetDevicePID, &infoValue);
                        usb_devices[1].devicePid = infoValue;
                        usb_echo("hid keyboard attached:pid=0x%x", infoValue);
                        USB_HostHelperGetPeripheralInformation(deviceHandle, kUSB_HostGetDeviceVID, &infoValue);
                        usb_devices[1].deviceVid = infoValue;
