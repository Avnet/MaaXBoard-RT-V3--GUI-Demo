 ...
 * Panel Definitions
 *
 *  FR = (BR x Lanes) / ((VACT + VSA + VBP + VFP) x (HACT + HSA + HBP + HFP) x Pixel Format
 *     = (BR x 2    ) / ((1280 + VSA + VBP + VFP) x (HACT + HSA + HBP + HFP) x Pixel Format
 *     =
 */
...
#define DEMO_HSW 8     // Horizontal Sync Width
#define DEMO_HFP 100   // Horizontal Sync Front Porch
#define DEMO_HBP 20    // Horizontal Sync Back Porch (blanking)
...
static status_t BOARD_InitLcdPanel(void)
{
    ...
    {
        GPIO_PinWrite(BOARD_MIPI_PANEL_BL_GPIO, BOARD_MIPI_PANEL_BL_PIN, 1);    // Turn-on the panel backlighting
    }
